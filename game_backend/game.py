...
class Game:
    def __init__(self, width=64, height=32):
        ...
        self._generator.gen_level()
        self._generator.gen_tiles_level()
        self._map2 = self._generator.tiles_level
        self.players = {}
        self.enemies = []
        self.sprites = []
    ...
    def add_item(self):
        ...
        y = random.randint(0,len(self._map)-1)
        found = False
        while found == False:
            y = random.randint(0,len(self._map)-1)
            tries = 0
            while True and tries<10: 
                x = random.randint(0,len(self._map[0])-1)
                tries += 1
                if self._map[y][x] == '.':
                    found = True
                    break
        if found:
            ...
            self._map[item_sprite.y][item_sprite.x] = item_sprite.content
        else:
            self.add_item()
